...
export default function ProdutosPage() {
  ...
  useEffect(() => {
    async function loadData() {
      // Usamos join para trazer o nome do grupo
      ...
      if (data) {
        setRows(data.map(item => ({
          ...
          preco: Number(item.preco_venda).toLocaleString('pt-BR', { minimumFractionDigits: 2 }),
          estoque: Number(item.estoque_atual).toLocaleString('pt-BR', { minimumFractionDigits: 3 }),
