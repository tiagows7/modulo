...
export default function DocumentosCaixaPage() {
  const { busy, pesquisar, gravar } = useDbStatus();
  const [items, setItems] = useState<DocumentoCaixa[]>([]);
  const [loadError, setLoadError] = useState("");
  const [actionError, setActionError] = useState("");
  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState<DocumentoCaixa | null>(null);
  const [deleting, setDeleting] = useState<DocumentoCaixa | null>(null);
  ...
  const loadData = useCallback(async () => {
    await pesquisar(async () => {
      ...
      setItems((data ?? []) as DocumentoCaixa[]);
    });
  }, [pesquisar]);
  ...
  const openEdit = (item: DocumentoCaixa) => {
    setEditing(item);
  ...
  const openDelete = (item: DocumentoCaixa) => {
    setDeleting(item);
    setActionError("");
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!desc) {
      setFormError("Informe a descrição do documento.");
      return;
    }
    ...
    try {
      await gravar(async () => {
        if (editing) {
          ...
          const { error } = await supabase.from("documentos_caixa").insert({
            codigo,
            descricao: desc,
            status: "ativo",
          });
  ...
  return (
    <>
      ...
      <ModulePage
        title="Documentos de Caixa"
        description="Formas de recebimento: dinheiro, cartão, PIX e outros"
      ...
      {modalOpen ? (
        <CadastroModal
          title={editing ? "Editar Documento de Caixa" : "Novo Documento de Caixa"}
          titleId="documento-caixa-title"
          subtitle={
            editing ? (
              <>
        ...
        >
          <CadastroField label="Descrição" htmlFor="documento-descricao">
            <input
      ...
      {deleting ? (
        <CadastroModal
          ...
          titleId="documento-caixa-delete-title"
          onClose={closeDelete}
          disabled={busy}
          width={400}
          footer={
